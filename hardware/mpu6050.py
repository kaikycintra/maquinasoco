...
if __name__ == "__main__":
    mpu = mpu6050(0x68)
    print("Reading data... Press Ctrl+C to stop.") # Initial message
    while True:
        try:
            time.sleep(0.1)
            accel_data = mpu.get_accel_data()

            # Format the data into a single string and use \r to return to the line's start
            output_string = (
                f"\rX: {accel_data['x']:.2f}, "
                f"Y: {accel_data['y']:.2f}, "
                f"Z: {accel_data['z']:.2f}"
            )
            
            # Print the string, using end='' to stay on the same line
